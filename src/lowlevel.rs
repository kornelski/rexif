...
pub(crate) fn read_u16_array(le: bool, count: u32, raw: &[u8]) -> Option<Vec<u16>> {
    Some(
        raw.get(..count as usize * 2)?
            .chunks_exact(2)
            .take(count as usize)
            .map(|ch| read_u16(le, ch).unwrap())
            .collect(),
    )
}
...
pub(crate) fn read_u32_array(le: bool, count: u32, raw: &[u8]) -> Option<Vec<u32>> {
    Some(
        raw.get(..count as usize * 4)?
            .chunks_exact(4)
            .take(count as usize)
            .map(|ch| read_u32(le, ch).unwrap())
            .collect(),
    )
}
...
pub(crate) fn read_f64_array(count: u32, raw: &[u8]) -> Option<Vec<f64>> {
    Some(
        raw.get(..count as usize * 8)?
            .chunks_exact(8)
            .take(count as usize)
            .map(|ch| read_f64(ch).unwrap())
            .collect(),
    )
}
...
pub(crate) fn read_urational_array(le: bool, count: u32, raw: &[u8]) -> Option<Vec<URational>> {
    Some(
        raw.get(..count as usize * 8)?
            .chunks_exact(8)
            .take(count as usize)
            .map(|ch| read_urational(le, ch).unwrap())
            .collect(),
    )
}
...
pub(crate) fn read_irational_array(le: bool, count: u32, raw: &[u8]) -> Option<Vec<IRational>> {
    Some(
        raw.get(..count as usize * 8)?
            .chunks_exact(8)
            .take(count as usize)
            .map(|ch| read_irational(le, ch).unwrap())
            .collect(),
    )
}
